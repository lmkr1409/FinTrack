...
class _LabelDialogState extends ConsumerState<LabelDialog> {
  ...
  String _transferType = 'DEBIT'; // 'DEBIT' (Outgoing), 'CREDIT' (Incoming), 'SELF' (Between accounts)
  ...
  void initState() {
    ...
    if (_transactionType == 'TRANSFER') {
      // Guess transfer type based on existing data if possible
      final cat = t.categoryId != null ? _categories.where((c) => c.id == t.categoryId).firstOrNull : null;
      if (cat?.categoryName == 'Self Transfer') {
        _transferType = 'SELF';
      } else {
        // Default to debit for investments/others
        _transferType = 'DEBIT';
      }
    }
  ...
  Future<void> _loadLookups() async {
    ...
    if (_expenseSourceId == null && (widget.transaction.description == null || widget.transaction.description!.trim().isEmpty)) {
      final manual = sources.where((s) => s.expenseSourceName == 'MANUAL_ENTRY').firstOrNull;
      if (manual != null) {
    ...
    if (_paymentMethodId != null && _paymentMethodKeyword.trim().isEmpty) {
      final kw = await tRuleRepo.getPatternByTypeAndId('PAYMENT_METHOD', 'payment_method_id', _paymentMethodId!);
      if (kw != null) _paymentMethodKeyword = kw;
    }
    if (_accountId != null && _accountKeyword.trim().isEmpty) {
      final kw = await tRuleRepo.getPatternByTypeAndId('ACCOUNT', 'account_id', _accountId!);
      if (kw != null) _accountKeyword = kw;
    }
    if (_cardId != null && _cardKeyword.trim().isEmpty) {
      final kw = await tRuleRepo.getPatternByTypeAndId('CARD', 'card_id', _cardId!);
  ...
  Future<void> _save() async {
    ...
    if (_transactionType == 'TRANSFER' && _transferType == 'SELF') {
      ...
      final otherId = await repo.insertTransaction(otherTxn);
      ...
      await repo.updateTransaction(
        widget.transaction.copyWith(
          ...
          paymentMethodId: fields['payment_method_id'],
          expenseSourceId: fields['expense_source_id'],
          purposeId: fields['purpose_id'],
          accountId: fields['account_id'],
          cardId: fields['card_id'],
          updatedTime: fields['updated_time'],
          labeled: true,
          isAutoLabeled: false,
          relatedTransactionId: otherId,
        ),
      );
    } else {
      // Standard labeling
    ...
    if (_saveAsRule) {
      ...
      // Amount Rule
      if (_amountStr.trim().isNotEmpty) {
        final escapedAmount = RegExp.escape(_amountStr.trim());
        await tRuleRepo.insert({
          'rule_type': 'AMOUNT_REGEX',
          'pattern': '($escapedAmount)',
          'updated_time': nowStr,
        });
      }
                ...
                : ListView(
                    ...
                    children: [
                      ...
                      Card(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text('Original Message', style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.primary, fontSize: 12)),
                              const SizedBox(height: 6),
                              Text(txn.description ?? 'No description', style: const TextStyle(fontSize: 14)),
                            ],
                          ),
                        ),
                      ),
                        ...
                        child: Column(
                          children: [
                            ...
                            ExpansionTile(
                              ...
                              leading: const Icon(Icons.numbers_rounded, color: AppColors.primary),
                              title: const Text('Amount Rule', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                              childrenPadding: const EdgeInsets.fromLTRB(4, 8, 4, 16),
                              children: [
                                TextFormField(
                                  initialValue: _amountStr,
                                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
                                  decoration: const InputDecoration(labelText: 'Amount', hintText: 'e.g., 500.00'),
                            ...
                            ExpansionTile(
                              ...
                              title: const Text('Transaction Type Rule', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                              childrenPadding: const EdgeInsets.fromLTRB(4, 8, 4, 16),
                              children: [
                                DropdownButtonFormField<String>(
                                  ...
                                  onChanged: (val) {
                                    if (val != null && val != _transactionType) {
                                      setState(() {
                                ...
                                TextFormField(
                                  ...
                                  decoration: const InputDecoration(labelText: 'Keyword used to identify type', hintText: 'e.g., debited'),
                                  onChanged: (v) => setState(() => _transactionTypeKeyword = v),
                                ),
                                if (_transactionType == 'TRANSFER') ...[
                            ...
                            ExpansionTile(
                              ...
                              title: const Text('Payment Method Rule', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                              childrenPadding: const EdgeInsets.fromLTRB(4, 8, 4, 16),
                              children: [
                                ...
                                TextFormField(
                                  ...
                                  decoration: const InputDecoration(labelText: 'Keyword used to identify method', hintText: 'e.g., UPI'),
                                  onChanged: (v) => setState(() => _paymentMethodKeyword = v),
                                ),
                              ],
                            ),
                            ...
                            ExpansionTile(
                              ...
                              title: const Text('Account Rule', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                              childrenPadding: const EdgeInsets.fromLTRB(4, 8, 4, 16),
                              children: [
                                AutocompleteField<Account>(
                                  label: 'Account',
                                ...
                                TextFormField(
                                  ...
                                  decoration: const InputDecoration(labelText: 'Keyword used to identify account', hintText: 'e.g., HDFC Bank'),
                                  onChanged: (v) => setState(() => _accountKeyword = v),
                                ),
                              ],
                            ),
                            ...
                            ExpansionTile(
                              ...
                              title: const Text('Card Rule', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                              childrenPadding: const EdgeInsets.fromLTRB(4, 8, 4, 16),
                              children: [
                                AutocompleteField<model.Card>(
                                  label: 'Card',
                                ...
                                TextFormField(
                                  ...
                                  decoration: const InputDecoration(labelText: 'Keyword used to identify card', hintText: 'e.g., ending with 1234'),
                                  onChanged: (v) => setState(() => _cardKeyword = v),
                                ),
                              ],
                            ),
                            ...
                            ExpansionTile(
                              ...
                              title: const Text('Merchant Rule', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                              childrenPadding: const EdgeInsets.fromLTRB(4, 8, 4, 16),
                              children: [
                                  ...
                                  optionsViewBuilder: (context, onSelected, options) {
                                    return Align(
                                      ...
                                      child: Material(
                                        ...
                                        child: SizedBox(
                                          width: MediaQuery.of(context).size.width - 32, // Parent width
                                ...
                                AutocompleteField<Merchant>(
                                  ...
                                  onAddNew: (text) {
                                    ...
                                    _addNew<Merchant>(
                                      'Merchant',
                                      text,
                                      (name) => ref.read(merchantRepositoryProvider).insert(Merchant(merchantName: name, priority: 99, iconColor: newColor, icon: 'store').toMap()).then((id) => Merchant(id: id, merchantName: name, priority: 99, iconColor: newColor, icon: 'store')),
                            ...
                            ExpansionTile(
                              tilePadding: const EdgeInsets.symmetric(horizontal: 4),
                              leading: const Icon(Icons.source, color: AppColors.primary),
                              title: const Text('Expense Source', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                              childrenPadding: const EdgeInsets.fromLTRB(4, 8, 4, 16),
                              children: [
                      ...
                      if (widget.showSaveRule)
                        Card(
                          ...
                          child: CheckboxListTile(
                            ...
                            value: _saveAsRule,
                            onChanged: (v) => setState(() => _saveAsRule = v ?? true),
                            title: const Text('Save as rule for future messages', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                            subtitle: const Text('New rules will be created automatically based on inputs above.', style: TextStyle(fontSize: 12)),
                      ...
                      FilledButton.icon(
                        ...
                        label: Text(_saving ? 'Saving…' : 'Save Label & Rules'),
